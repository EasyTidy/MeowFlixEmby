...
func (a *App) buildPlayers() *player.MapRegistry {
	players := []player.Player{
		mpv.New(mpv.Options{ExePath: a.cfg.Players.Exe["mpv"]}),
		vlc.New(vlc.Options{ExePath: a.cfg.Players.Exe["vlc"]}),
		potplayer.New(potplayer.Options{ExePath: a.cfg.Players.Exe["potplayer"]}),
		mpc.New(mpc.Options{ExePath: a.cfg.Players.Exe["mpc-hc"]}),
	}
	// A generic launch-only driver for any other configured executable.
	if exe := a.cfg.Players.Exe["generic"]; exe != "" {
		players = append(players, generic.New(generic.Options{ExePath: exe}))
	}
