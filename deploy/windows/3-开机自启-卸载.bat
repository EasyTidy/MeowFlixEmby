@echo off
rem ---------------------------------------------------------------------------
rem MeowFlixEmby - remove the per-user login autostart entry (double-click me).
rem Pure ASCII by design; Chinese interaction lives in guide.ps1.
rem ---------------------------------------------------------------------------
chcp 65001 >nul
setlocal
title MeowFlixEmby - Remove autostart

call :find_guide || goto :fail
powershell -NoProfile -ExecutionPolicy Bypass -File "%GUIDE%" -Action autostart-uninstall -ShimDir "%~dp0"
set "CODE=%ERRORLEVEL%"
